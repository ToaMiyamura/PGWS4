...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	depthResDesc.SampleDesc.Count = 1; //サンプルは1、ピクセルあたり1つ
	depthResDesc.Flags =
		D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL; // デプスステンシルとして使用

	//深度値用ヒーププロパティ
	D3D12_HEAP_PROPERTIES depthHeapProp = {};
	depthHeapProp.Type = D3D12_HEAP_TYPE_DEFAULT;//DEFAULTなので後はUNKNOWN
	...
	result = _dev->CreateCommittedResource(
		...
		&depthResDesc,
		D3D12_RESOURCE_STATE_DEPTH_WRITE,//深度値書きこみに使用
		&depthClearValue,
...
#pragma pack(pop)
	...
	matDescHeapDesc.NumDescriptors = materialNum*2; // マテリアル数を指定
	...
	struct MatricesData
	{
		XMMATRIX world;//モデル本体を回転、移動させる行列
